...
This is a guide for setting up Google Cloud Engine for the course. I have obviously stolen a lot from the [original course guide](http://cs231n.github.io/gce-tutorial/), but this is intended to be a standalone guide. We realize people come from different backgrounds and were are here to help those who might have difficulties with getting everything up and running. Feel free to talk to us after class and/or use the [Facebook group](https://www.facebook.com/groups/553903248285761/).

## Before We Begin: Credits/Stopping Your Instances
Google was nice enough to provide each one of you with 500$ worth of *Google Cloud Platform* (*GCP* from now on) credits. This might sound like a lot, but in reality it actually isn't that much. The most important thing to remember during this course - and when working with cloud instances in general - is top **stop them when unused**. I'll repeat this; 
...
The place where magical, free credits exist is [here.](https://docs.google.com/forms/d/e/1FAIpQLSfdhuK9cP2RIMjVybxAb5ROuuIOljF7rlrpOUumFb0Ui9fFsA/viewform?entry.204193097&entry.126615916&entry.1808741675&entry.1107548265=Israel&entry.649475960=29683)

You should enter **only using the email address associated with the Google account you will be using!**. For *Comapny Legal Name* use **your own name**, and for the domain put your school domain as a placeholder. For *Contact Name* enter **your** name *again*.
...
![first time]({{ site.baseurl }}/images/credits.png)

For **Are you currently using Google Cloud Platform?** enter 'No', unless *are* currently using it!

Once done with this, please let us know what e-mail address you used for registration by filling this [very simple form](https://goo.gl/forms/lnsrpVfBDfYNme8I3). Without doing this your e-mail won't be approved. 
...
- Machine type; pick **4 vCPUs** from the dropdown menu.
...
Make sure to click the **Customize** button and make sure that **Number of GPUs** is set to **None** - but that you have the option to add a GPU later on! The instance should have 4 CPUs and 15GB of RAM by default. Leave that as is.

Now, click the blue **Management, disks, networking, SSH keys** link and more options will load up. In the new options, click the **Disks** tab. Uncheck **Delete boot disk when instance is deleted**. Finally, we can hit the blue **Create** button and launch our instance (can take a mintue).
...
We now config the SDK by running the final `./google-cloud-sdk/bin/gcloud init`  command.

Note that you should run this from the directory where the package was extracted (Linux/macOS users) or from the installation folder (Windows). In the windows case, the command should simply be `gcloud init`.
...
We are using `us-west1-b`, so hit `38` and click Enter. This part is done.
...
**Make sure you did the Locale fix before running this part!**

##### New: Script Method:
I wrote an experimental script which should do the following steps. Run it *inside your instance* by executing:
...
##### Fixing Locale Error
Python sometimes complains about some bad locale. This error would look like this:
...
If you get that error during script execution, execute the following 3 lines before running the script. In the menu that loads up simply hit **Enter** until done. 
...
Once inside the instance, we will need to execute the following set of commands in order to get our environment ready:
...
- Install `unzip` and use it to unzip the file:
...
- Go into the unzipped directory and execute the installation script:

```bash
cd assignment1 
./setup_googlecloud.sh
```
This will involve hitting `Y` a few times (once for every installed package-set basically). 

##### Launching Jupyter

- Assuming no errors, we will be able to load our *virtual environment* by executing:
